...
- #### 基本介绍
  ...
  2. 默认情况下， atd 守护进程每 60 秒检查作业列队，有作业时，会检查作业运行时间，如果时间与当前时间匹配，则运行此作业
  3. at命令是一次性定时计划任务，执行完一个任务后不再执行此任务
  4. 在使用 at 指令的时候，一定要保证 atd 进程的启动，可以使用相关指令来查看

- #### 基本语法

  - **at [选项] [时间]**
    - ctrl + D 结束 at 命令输入**（输入两次）**
    - **时间定义：**
      - hh:mm（当天），若输入时间为已过去的时间，将会再第二天执行**（04:00）**
      - midnight，noon，teatime
      - 12小时计时制**（12pm）**
      - 具体日期 month day 或者  mm/dd/yy 或者 dd.mm.yy，指定日期后必须跟在指定时间后**（04:00 2022-3-28）**
      - now + count time - units**（now + 5 minutes）**
      - today tomorrow
...
- #### 直接修改配置文件
  ...
  - ![image-20220330172858489](C:\Users\Hu\AppData\Roaming\Typora\typora-user-images\image-20220330172858489.png)
...
- **过程：**
  ...
  1. 浏览器首先检查 **浏览器缓存中，有没有该域名解析IP地址**
  2. **如果没有则 检查 DNS 解析器缓存（本地解析器缓存）**
     1. ipconfig /displaydns    显示DNS域名缓存
     2. ipconfig /flushdns    手动清理缓存

  3. 如果本地解析器没有，检查系统中 host 文件中有没有对应的域名IP
  4. 如果 host 也没有，就会到 域名服务的 DNS  进行解析

## 十四、进程管理

### 显示系统执行的进程

- **ps 命令 是用来查看目前系统进程中，有哪些程序在执行，以及执行状况**
  - **pid：**进程识别号
  - **TTY：**终端机号
  - **TIME：**此进程所消耗的CPU时间
  - **CMD：**正在执行的命令或者进程名
